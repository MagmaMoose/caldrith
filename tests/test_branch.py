...
async def test_detects_review_count_drift() -> None:
    ...
    cfg = BranchConfig(
        ...
        protection=BranchProtection(
            ...
            required_status_checks=RequiredStatusChecks(strict=True, contexts=["ci/build"]),
            required_pull_request_reviews=RequiredPullRequestReviews(
                dismiss_stale_reviews=True, required_approving_review_count=1  # want 1, live is 2
            ),
        ),
    )
    async with GitHub("token") as client:
...
async def test_default_branch_resolved() -> None:
    respx.get("https://api.github.com/repos/acme/widget").mock(
        return_value=httpx.Response(
            200, json={"name": "widget", "default_branch": "develop", "owner": {"login": "acme"}}
        )
    )
    prot = respx.get(
        "https://api.github.com/repos/acme/widget/branches/develop/protection"
    ).mock(return_value=_not_protected())
    put = respx.put(
        "https://api.github.com/repos/acme/widget/branches/develop/protection"
    ).mock(return_value=httpx.Response(200, json=_live()))
